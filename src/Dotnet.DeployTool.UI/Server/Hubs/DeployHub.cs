...
namespace Dotnet.DeployTool.UI.Server.Hubs
{
    public class DeployHub : Hub
    {
        ...
        {
            await _deployService.SetupService(pemFilePath, ip, port, username, projectName, dllName);
        }

        public async Task RunSample(string pemFilePath, string ip, int port, string username, string projectName, string dllName)
        {
            await _deployService.RunSample(pemFilePath, ip, port, username, projectName, dllName);
        }
